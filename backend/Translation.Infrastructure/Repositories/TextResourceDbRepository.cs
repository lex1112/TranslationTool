...
{
    public sealed class TextResourceDbRepository : ITextResourceRepository
    {
        private readonly ApplicationDbContext _context;

        public TextResourceDbRepository(ApplicationDbContext context) => _context = context;

        // Get all SIDs (Business Keys)
        public async Task<IEnumerable<TextResourceEntity>> GetAllTextResource()
            => await _context.TextResources
                .Include(x => x.Translations)
                .ToListAsync();

        // Get the full Aggregate (Resource + Translations)
        ...
        // Unit of Work: Persist all changes
